...
enum class FieldInputType(val holderType: Int) {
    EDIT_TEXT(1) {
        override fun getViewHolder(parent: ViewGroup, settings: Settings) : BaseViewHolder {
            val binding: FieldEditTextBinding = DataBindingUtil.inflate(LayoutInflater.from(parent.context), R.layout.field_edit_text, parent, false)
...
//    }
    ...
    abstract fun getViewHolder(parent: ViewGroup, settings: Settings) : BaseViewHolder
...
//                is DividerField -> DIVIDER
        ...
        fun getViewHolder(type: Int, parent: ViewGroup, settings: Settings): BaseViewHolder {
            values().map {
                if (it.holderType == type) {
                    return it.getViewHolder(parent, settings)
                }
            }
            return DIVIDER.getViewHolder(parent, settings)
        }
    }
}